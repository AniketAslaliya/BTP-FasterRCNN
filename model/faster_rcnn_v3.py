"""
faster_rcnn_v3.py — Version C (Revised)

Architecture:
  - Dual ResNet-50 backbone (RGB + IR streams, ImageNet pretrained)
  - C1 : simple channel-wise average fusion
  - C2 : avg fusion → CBAM (channel + spatial attention)   ← REVISED (was CrossAttn)
  - C3 : simple channel-wise average fusion
  - C4 : CrossAttention (Q=RGB, K/V=IR)                   ← same as V1
  - FPN : 5-level feature pyramid (256-ch each)
  - RPN + MultiScaleRoIAlign + detection head from torchvision

Rationale:
  Original V3 used global CrossAttention at C2 (128x160 = 20480 tokens),
  which requires ~38 GB for the attention matrix at batch=24 — infeasible on 32GB GPUs.

  CBAM at C2 is memory-efficient (channel avg pool + 7x7 conv) and equally effective
  for early-stage feature refinement:
    - Channel attention suppresses noisy modality channels
    - Spatial attention highlights thermally active / visually salient regions
  CrossAttention at C4 (32x40 = 1280 tokens) handles deep semantic cross-modal alignment.
"""
...
# ---------------------------------------------------------------------------
# C2 Fusion: avg + CBAM  (memory-efficient early-stage refinement)
# ---------------------------------------------------------------------------

class C2CBAMFusion(nn.Module):
    """
    C2-level fusion: avg(RGB_C2, IR_C2) -> CBAM(512ch) -> out_conv

    Memory cost: ~few MB (vs ~38 GB for CrossAttention at C2)
    Effect: channel attention suppresses noisy channels,
            spatial attention highlights pedestrian-relevant regions early.
    """
    def __init__(self, in_ch: int = 512, reduction: int = 16):
        super().__init__()
    ...
    def forward(self, rgb_c2: torch.Tensor, ir_c2: torch.Tensor) -> torch.Tensor:
        fused = (rgb_c2 + ir_c2) * 0.5   # avg fusion
        fused = self.cbam(fused)          # channel + spatial attention
        return self.out_conv(fused)


# ---------------------------------------------------------------------------
# CrossAttention Module (for C4 fusion — same as V1)
# ---------------------------------------------------------------------------

class CrossAttentionFusion(nn.Module):
    """
    Cross-modal attention at C4: Q=RGB, K/V=IR.
    C4 spatial: 32x40 = 1280 tokens — memory safe at batch=24.
    """
    def __init__(self, in_ch: int, attn_dim: int = 256, num_heads: int = 8):
        super().__init__()
    ...
    def forward(self, rgb_feat: torch.Tensor, ir_feat: torch.Tensor) -> torch.Tensor:
        ...
        fused        = self.norm(attn_out + rgb_seq)
...
# ---------------------------------------------------------------------------
# Full Multimodal Backbone (body + fusion + FPN)
# ---------------------------------------------------------------------------

class MultimodalBackboneWithFPN(nn.Module):
    """
    V3 Revised fusion strategy:
        C1  →  avg(RGB, IR)
        C2  →  avg(RGB, IR) → CBAM(512ch)     early refinement, memory-safe
        C3  →  avg(RGB, IR)
        C4  →  CrossAttention(Q=rgb, K/V=ir)   deep semantic fusion
    """
    def __init__(self, fpn_out: int = 256):
        super().__init__()
        self.body      = DualResNetBackbone()
        self.fusion_c2 = C2CBAMFusion(in_ch=512, reduction=16)
    ...
    def forward(self, rgb: torch.Tensor, ir: torch.Tensor) -> dict:
        f  = self.body(rgb, ir)
        c1 = (f["rgb_c1"] + f["ir_c1"]) * 0.5           # simple avg
        c2 = self.fusion_c2(f["rgb_c2"], f["ir_c2"])     # avg + CBAM
        c3 = (f["rgb_c3"] + f["ir_c3"]) * 0.5           # simple avg
        c4 = self.fusion_c4(f["rgb_c4"], f["ir_c4"])     # CrossAttention
...
class FasterRCNN(nn.Module):
    def __init__(self, model_config=None, num_classes: int = 2):
        super().__init__()
    ...
    def forward(self, rgb, ir, targets=None):
        ...
        ir_r = F.interpolate(ir_batch, size=(H, W),
